 ...
 */
public class InputMarksController extends BaseController {
    ...
    @Override
    public void refresh() {
        int max = EXAM.getLastCode();
        SpinnerInitializer.initialize(firstCodeSP, new SpinnerValueFactory.IntegerSpinnerValueFactory(EXAM.getFirstCode(), max, EXAM.getFirstCode()));
        SpinnerInitializer.initialize(lastCodeSP, new SpinnerValueFactory.IntegerSpinnerValueFactory(EXAM.getFirstCode(), max, max));
        firstCodeSP.getValueFactory().setValue(EXAM.getFirstCode());
        lastCodeSP.getValueFactory().setValue(max);
    ...
    private void createForm() {
        ...
        if (READY.get()) {
            EXAM.getStudents().stream()
                    .filter(stu -> ((StudentExamParameters) stu.getUserData()).getExamCode() >= firstCodeSP.getValue()
                    && ((StudentExamParameters) stu.getUserData()).getExamCode() <= lastCodeSP.getValue())
                    ...
                    .forEach(stu -> {
                        Platform.runLater(() -> {
                            formLV.getItems().add(new MarkInput(stu, EXAM.getType(), matterCB.getValue().getLabel()));
                        });
                    });
        }
    }
